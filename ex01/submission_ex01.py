...
The "NAME" file as well as the "src" and "doc" directories need to
be located at root of the archive! Also, do not forget to 'check'
and 'test' your solutions first using this tool ;)
'''

PYTHON = "python3"
PYERROR = "For running your solution we call '{}'.\nThe name might different for your installation (e.g. on Windows)\n"
...
def add_student(firstnames, surname, student_id, output, max_nstudents=1, errors_only=False):
    ...
    try:
        with open(os.path.join(output, NAME_FILE), 'r', encoding='utf-8', errors='ignore') as f:
            n = sum(1 for _ in f)
    except:
        n = 0
    ...
    try:
        ...
        with open(os.path.join(output, NAME_FILE), 'a', encoding='utf-8', errors='ignore') as f:
    ...
    try:
        with tarfile.open(archive, 'r:gz') as f:
            ...
            try:
                ...
                error('The file needs to be located at the root of the archive')
                return -2
...
def test_submission(archive, valscripts, output, rm_output, unpack_only=False):
    ...
    def include(ti):
        ...
        if path.startswith(SRC_DIR) and path.endswith(".txt"):
             return True
        # if path.startswith(SRC_DIR) and path.endswith(".url"):
        #     return True
        # if path.startswith(SRC_DIR) and path.endswith(".sh"):
        #     return True
        if path == NAME_FILE:
            return True
    ...
    with open(student_name, 'r', encoding='utf-8', errors='ignore') as f:
    ...
    if valscripts:
        ...
        if not unpack_only:
            ...
            os.chdir(src)
            ret = subprocess.call("{} -B {}".format(PYTHON, test), shell=True)

            if ret != 0:
                error(PYERROR.format(PYTHON))
